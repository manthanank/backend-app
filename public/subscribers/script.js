document.addEventListener("DOMContentLoaded", function () {
    ...
    function handleTableClick(event) {
        ...
        if (actionElement) {
            ...
            if (subscriberId && confirm('Are you sure you want to delete this subscriber?')) {
    ...
    async function fetchAndPopulateSubscribers(page) {
        try {
            ...
            const apiEndpoint = isLocal
                ...
                : "https://backend-app-8ev9.onrender.com/api/subscribers";
    ...
    async function deleteSubscriber(subscriberId) {
        try {
            ...
            if (response.ok) {
                ...
                const deletedRow = document.querySelector(`[data-subscriber-id="${subscriberId}"]`);
                if (deletedRow) {
                    deletedRow.remove();
                }
